 ...
 @Test
  ...
  public void beforeWithRemoveBST() {
    BinarySearchTree<Integer> bst 
      = new BinarySearchTree<>((Integer x, Integer y) -> x < y);
    assertTrue(bst.isEmpty());
  ...
  @Test
  public void afterWithRemoveBST() {
    BinarySearchTree<Integer> bst 
      = new BinarySearchTree<>((Integer x, Integer y) -> x < y);
    assertTrue(bst.isEmpty());
    for (int key = 1; key <= 100; key++)
    ...
    for (int key = 2; key <= 100; key += 2)
      bst.remove(key);
    for (int key = 1; key <= 97; key += 2) 
      assertTrue(key + 2 == bst.search(key).getAfter().get());
  }
